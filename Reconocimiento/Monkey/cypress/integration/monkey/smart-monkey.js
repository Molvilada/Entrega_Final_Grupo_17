  ...
  cy.window().then((win) => {
    ...
    if (!!element) {
      if (element.hasAttribute("onmouseover")) {
        ...
        info = `Position: (${randX}, ${randY}). INVALID, element has no attribute onmouseover`;
    } else
      info = `Position: (${randX}, ${randY}). INVALID, no selectable element`;
...
function avPag() {
  ...
  if (
    curPageMaxY - curY >= viewportHeight &&
    curY + viewportHeight < pageHeight
  ) {
...
function horizontalScrollFw() {
  ...
  cy.task("logCommand", {
    funtype: "Scroll event (horizontal fw)",
    info: info,
  });
}
...
function horizontalScrollBk() {
  ...
  cy.task("logCommand", {
    funtype: "Scroll event (horizontal bk)",
    info: info,
  });
}
...
describe(`${appName} under smarter monkeys`, function () {
  ...
  cy.on("window:alert", (text) => {
    cy.task("genericLog", {
      message: `An alert was fired with the message: "${text}"`,
    });
    cy.task("genericReport", {
  ...
  cy.on("fail", (err) => {
    cy.task("genericLog", {
      message: `The test failed with the following error: ${err}`,
    });
    cy.task("genericReport", {
